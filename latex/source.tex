...
\begin{document}
    ...
    some loss function $\mathcalL(\pmb{\theta})$ with respect to (wrt.) the model's parameters $\pmb{\theta}$.
    ...
    which as is is apparent, depends indirectly on parameters $\theta$.
    \begin{equation}
        L(\pmb{y}(T), \pmb{y}^*) = L \left( \pmb{y}(0) +\int_{0}^{T} f(\pmb{y}(\tau), \tau, \pmb{\theta}) \,d\tau \right, \pmb{y}^*)
        \label{eq:ode_loss}
    \end{equation}
    ...
    \begin{align*}
        ...
        y(t_n+h) &= y_{n+1}  + \frac{1}{2} h^2 y''(t_n) + \mathcal{O}(h^3) \\
        y(t_n+h) - y_{n+1} &= \frac{1}{2} h^2 y''(t_n) + \mathcal{O}(h^3) \\
    ...
    \begin{equation}
        ...
        \label{eq:tayl2}
    \end{equation}
    We now apply the 2-dimensional Taylor expansion on the right-hand side of ~\eqref{eq:sol:tayl2}:
    \begin{align*}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            \Repeat
                ...
                \Else
                    \State Evaluate $\phi'(a_j)$
                    \If {$ |\phi'(a_j)| \leq -c_2 \phi'(0)$}
                        \State Set $a^* \gets a_j$ and \textbf{stop}
                    \EndIf
                    \If { $\phi'(a_j)(a_{hi} - a_{lo}) \geq 0 $ }
